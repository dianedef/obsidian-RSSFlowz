import { Plugin } from 'obsidian'

export class RegisterStyles {
   constructor(private plugin: Plugin) {}

   register(): void {
      // Ajouter ces styles CSS
      document.head.appendChild(Object.assign(document.createElement('style'), {
         id: 'rss-reader-styles',
         textContent: `
            /* Styles pour la vue de lecture */
            .rss-reader-container {
               padding: 16px;
            }

            .rss-reader-header {
               display: flex;
               justify-content: space-between;
               align-items: center;
               margin-bottom: 16px;
               padding-bottom: 8px;
               border-bottom: 1px solid var(--background-modifier-border);
            }

            .rss-reader-nav-buttons {
               display: flex;
               gap: 8px;
            }

            .rss-reader-nav-buttons button {
               padding: 4px 12px;
               border-radius: 4px;
               background: var(--interactive-accent);
               color: var(--text-on-accent);
               border: none;
               cursor: pointer;
               transition: background-color 0.2s ease;
            }

            .rss-reader-nav-buttons button:hover {
               background: var(--interactive-accent-hover);
            }

            .rss-reader-content {
               line-height: 1.6;
            }

            /* Styles pour la modal de lecture */
            .reading-mode-active {
               max-width: 800px;
               margin: 0 auto;
               padding: 20px;
               line-height: 1.6;
            }

            .reading-mode-active h1 {
               margin-bottom: 24px;
               padding-bottom: 12px;
               border-bottom: 1px solid var(--background-modifier-border);
            }

            .reading-mode-controls {
               position: fixed;
               bottom: 20px;
               left: 50%;
               transform: translateX(-50%);
               background: var(--background-primary);
               padding: 10px 20px;
               border-radius: 8px;
               box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);
               display: flex;
               gap: 10px;
               align-items: center;
               z-index: 1000;
            }

            .reading-mode-select {
               padding: 4px 8px;
               border-radius: 4px;
               border: 1px solid var(--background-modifier-border);
               background: var(--background-primary);
               color: var(--text-normal);
            }

            .reading-mode-controls button {
               padding: 4px 12px;
               border-radius: 4px;
               background: var(--interactive-accent);
               color: var(--text-on-accent);
               border: none;
               cursor: pointer;
               transition: background-color 0.2s ease;
            }

            .reading-mode-controls button:hover {
               background: var(--interactive-accent-hover);
            }

            /* Styles pour le contenu des articles */
            .reading-mode-active img {
               max-width: 100%;
               height: auto;
               margin: 16px 0;
               border-radius: 4px;
            }

            .reading-mode-active blockquote {
               margin: 16px 0;
               padding: 8px 16px;
               border-left: 4px solid var(--interactive-accent);
               background: var(--background-primary-alt);
               border-radius: 0 4px 4px 0;
            }

            .reading-mode-active pre {
               margin: 16px 0;
               padding: 16px;
               background: var(--background-primary-alt);
               border-radius: 4px;
               overflow-x: auto;
            }

            .reading-mode-active code {
               font-family: var(--font-monospace);
               font-size: 0.9em;
               padding: 2px 4px;
               background: var(--background-primary-alt);
               border-radius: 3px;
            }

            .reading-mode-active a {
               color: var(--interactive-accent);
               text-decoration: none;
            }

            .reading-mode-active a:hover {
               text-decoration: underline;
            }
         `
      }));
   }

   unregister(): void {
      const styleElement = document.getElementById('rss-reader-styles');
      if (styleElement) {
         styleElement.remove();
      }
   }
}
